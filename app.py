...
if params.get('page') == ['math_appendix']:
    ...
    st.markdown("When 'multiple simulations' is selected randomization occurs over log-uniform distributions for $f$, $\tau$, $\lambda$, and $D$. When you sellect 'display empirical distributions' in the side bar, on runing a simulation, you will also get a histogram of the values used in the simulation.")
